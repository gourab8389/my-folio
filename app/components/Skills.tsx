    ...
    <div className="px-10 py-10 md:py-20" id="skills">
      ...
      <div className="grid md:grid-cols-4 lg:grid-cols-6 grid-cols-2 gap-4 w-full lg:w-4/5 mx-auto my-[3rem]">
        {skills.map((item) => (
          ...
          <div key={item.id} className="flex gap-4 p-4 rounded-lg justify-center items-center bg-gray-50 dark:bg-black shadow-md hover:bg-slate-100 dark:hover:bg-slate-950 border dark:border-gray-950">
            <SkillDataProvider 
            key={item.id}
              src={item.image} 
              width={30} 
